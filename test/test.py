...
def performAction(black_centroid, action,drag, perform):
    if perform:
...
def main(black_range):
    ...
    while True:
        ...
        hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        #print("Lower bound:", black_range[0])
        #print("Upper bound:", black_range[1])
        black_mask = makeMask(hsv, black_range)
        #target = cv2.bitwise_and(hsv, hsv, mask = black_mask)
        #print("Target: ", target)
        black_cen = drawCentroid(frame, black_area, black_mask, showCentroid)
        prediction = handDetect(finger_detect.detect())
        #print("Prediction: ", prediction)
        output = chooseAction(prediction)
        if output[0] != -1:
            performAction(black_cen, output[0], output[1], perform)
